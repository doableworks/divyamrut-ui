...
const CartPage = () => {
  ...
  const handleIncreaseCartItem = async (item, action) => {
    try {
      let response;
      if (session) {
        response = await IncreAndDecreCartItemQuantity(action, item.product.uid);
      }
  ...
  const calculateTotal = () => {
    return items
      ?.reduce(
        (total, item) =>
          total + parseFloat(item.product.price.replace(/,/g, "")) * Number(item.quantity),
        0
      )
      .toFixed(2);
  };
              ...
              >
                ...
                <div className="flex flex-1 flex-row items-start">
                  ...
                  <p className="hidden md:block section-title">
                    ₹&nbsp;
                    {(
                     parseFloat(item.product.price.replace(/,/g, "")) * Number(item.quantity)
                    ).toFixed(2)}
                  </p>
                </div>
              </div>
