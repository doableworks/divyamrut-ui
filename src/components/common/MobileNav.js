...
import { Arrow, ThreeLine } from "@/icon/icons";

const MobileNav = ({pathname, handleMoveRoute}) => {
    const [openMobileNav, setOpenMobileNav] = useState(false);
    const [itemOpen, setItemOpen] = useState({products : false,  advanceTherapies: false});
     ...
     <button
        ...
        {openMobileNav ? (
          <nav className="absolute top-24 left-0 w-screen bg-text flex flex-col gap-2 font-jost text-[16px] md:text-[18px] font-[500] text-left text-a2c0d56 shadow-lg">
            ...
            onClick={()=>setItemOpen({... itemOpen, products:!itemOpen.products})}
            >
              <div className={`cursor-pointer ${
                    itemOpen.products ? "text-text bg-q4ca25af"
                  : "text-a2c0d56"
                  }`}>
                <h5
            ...
            <h5
              className={`cursor-pointer  py-2 px-5  ${
                pathname == "/purpose" ? "text-text bg-q4ca25af"
                  : "text-a2c0d56"
              }`}
            ...
            <h5
              className={`cursor-pointer py-2 px-5  ${
                pathname == "/contact-us" ? "text-text bg-q4ca25af"
                  : "text-a2c0d56"
              }`}
            ...
            //   onMouseEnter={() => setHasHover("Pages")}
            //   onMouseLeave={() => setHasHover(null)}
            onClick={()=>setItemOpen({... itemOpen, advanceTherapies:!itemOpen.advanceTherapies})}
            >
              <div className={`cursor-pointer ${
                    itemOpen.advanceTherapies  ? "text-text bg-q4ca25af"
                  : "text-a2c0d56"
                  }`}>
                <h5
                  className={`flex items-center py-2 pb-4 px-5 cursor-pointer`}
                >
                  Advance Therapies
                  <Arrow fill={itemOpen.advanceTherapies ? "#FFFFFF" : "#99C24A"} />
                </h5>
              </div>
              {/* Dropdown */}
             {itemOpen.advanceTherapies && <div
