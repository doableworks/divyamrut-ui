...
} from "@/redux/feature/buyProductSlice";
...
import { clearCart, removeCartItem } from "@/redux/feature/cartSlice";
...
const BuyProductCom = ({ allAddressData }) => {
  ...
  const [addOrEditAddre, setADDorEditAddre] = useState({
    open: allAddressData.length > 0 ? false : true,
    address: null,
  });
  ...
  const { removeCartItemAPI, updateInventoryOnPurchaseAPI } = useCart();
  ...
  const onFillAddressFinish = async (values) => {
    ...
    try {
      const response = await axios.post(
        process.env.NEXT_PUBLIC_API_URL + "/product/shipping/address/",
        data
      );
    ...
    try {
      const response = await axios.put(
        process.env.NEXT_PUBLIC_API_URL +
          `/product/shipping/address/${addOrEditAddre.address.uid}/`,
        data
      );
      if (response.status === 200) {
        const data = response?.data?.data;
        const remainAddress = addressList.filter(
          (item) => item.uid != addOrEditAddre.address.uid
        );
  ...
  const onDeleteAddress = async (uid, email) => {
    try {
      ...
      const response = await axios.post(
        process.env.NEXT_PUBLIC_API_URL +
          `/product/shipping/address/delete/${uid}/`,
        data
      );
        ...
        () =>
          messageApi.open({
            ...
            content:
              ...
              "Unable to create order, Please try after sometime.",
          }),
        300
      );
    } finally {
  ...
  const handleCreateOrder = async (data) => {
    ...
    try {
      const requestData = {
        ...
        notes: {
          user_email:
            session?.user?.user?.email || SelectedDeliveryAddre?.email,
          email: SelectedDeliveryAddre?.email,
          therapist_id: "",
          product_id: "",
          therapy_slug: "",
  ...
  const proceedToOrderFnCall = async () => {
    ...
    try {
      ...
      const productUids = orderList.map((item) => item.product_detail.uid);
      const inventoryData = orderList.map((item) => ({
        uid: item.product_detail.uid,
      ...
      const options = {
        ...
        name:
          session?.user?.user?.first_name || SelectedDeliveryAddre?.first_name,
        ...
        prefill: {
          name:
            ...
            SelectedDeliveryAddre?.first_name +
              SelectedDeliveryAddre?.last_name,
