...
const SelectDeliveryAddress = ({
  addressList,
  SelectedDeliveryAddre,
  handleAddNeworEditAddre,
  onDeleteAddress,
  onSelectAddress,
}) => {
  return (
    <div className="w-full mx-auto p-4 space-y-8">
      <h2 className="text-xl font-semibold text-[--yellow]">Select Delivery Address</h2>
      <Divider />
      <CustomButton
        htmlType="submit"
        className="site-button-primary w-[200px] capitalize"
        title="Add New Address"
        loading={false}
        icon={<PlusOutlined />}
        onClick={() => handleAddNeworEditAddre(true, null)}
      />
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full flex-wrap">
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                <Radio
                  value={addr.uid}
                  checked={SelectedDeliveryAddre?.uid == addr.uid}
                />
                <div>
                  ...
                  <div className="mt-3 text-gray-700 space-y-1">
                    ...
                    <p className="section-content !text-left">
                      <strong>Phone number:</strong> {addr?.phone}
                    </p>
                  </div>
                </div>
              </div>
