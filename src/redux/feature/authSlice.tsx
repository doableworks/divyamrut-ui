...
export const fetchUserData = createAsyncThunk(
  'auth/fetchUserData',
  async (session: any, { dispatch, rejectWithValue }) => {
    try {
      if (session) {
        const response = await axiosInstance.get('api/account/auth/user/', { session });
        const userData = response.data;
...
export const getFree = (router: any) => async (dispatch: AppDispatch) => {
  try {
    dispatch(setLoadingSubscription(true));
    const session = await getSession();
    const response = await axiosInstance.post("api/document/trial/", {}, { session });
